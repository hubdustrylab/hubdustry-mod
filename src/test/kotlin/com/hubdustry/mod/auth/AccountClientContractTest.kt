    ...
    @Test fun pairCollectRefreshLogoutUsesCanonicalHeadersAndFencesLogout() {
        ...
        server.createContext("/") { exchange ->
            ...
            val body = when {
                ...
                path.endsWith("/collect") -> """{"sessionId":"00000000-0000-0000-0000-000000000001","accessToken":"access-token-123456","refreshToken":"refresh-token-123456","accessExpiresAt":"2026-01-01T00:00:00Z","refreshExpiresAt":"2026-02-01T00:00:00Z","account":{"id":"user","displayName":"Test","avatarRef":"https://cdn.discordapp.com/avatars/123/0123456789abcdef0123456789abcdef.png"}}"""
        ...
        try {
            ...
            assertNotNull(client.session); val avatar = client.session?.avatarRef; assertNotNull(avatar)
            val refreshLatch = CountDownLatch(1); client.refresh { assertTrue(it.isSuccess); refreshLatch.countDown() }; assertTrue(refreshLatch.await(2, java.util.concurrent.TimeUnit.SECONDS))
            assertEquals(avatar, client.session?.avatarRef)
